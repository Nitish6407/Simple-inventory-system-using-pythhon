...
root = Tk()
root.title("Python: Billing Inventory System :--CSDT")
...
def Database():
    ...
    cursor.execute("SELECT * FROM `admin` WHERE `username` = 'nitish' AND `password` = 'nitish'")
    if cursor.fetchone() is None:
        cursor.execute("INSERT INTO `admin` (username, password) VALUES('nitish', 'nitish')")
        conn.commit()

def Exit():
    result = tkMessageBox.askquestion('Billing Inventory System', 'Are you sure you want to exit?', icon="warning")
    if result == 'yes':
        root.destroy()
        exit()
...
def ShowLoginForm():
    global loginform
    loginform = Toplevel()
    loginform.title("Billing Inventory System/Account Login")
    width = 600
    height = 500
    screen_width = root.winfo_screenwidth()
    screen_height = root.winfo_screenheight()
...
def Home():
    global Home
    Home = Tk()
    Home.title("Billing Inventory System Disign By CSDT/Home")
    width = 1024
    height = 720
    ...
    Title.pack(pady=10)
    lbl_display = Label(Title, text="Billing Inventory System Disign By CSDT", font=('arial', 24))
    lbl_display.pack()
    menubar = Menu(Home)
    ...
    filemenu2.add_command(label="Add new Product", command=ShowAddNew)
    filemenu2.add_command(label="Stock Details", command=ShowView)
...
def ShowAddNew():
    global addnewform
    addnewform = Toplevel()
    addnewform.title("Billing Inventory System Disign By CSDT/Add new")
    width = 600
    height = 500
    screen_width = Home.winfo_screenwidth()
    screen_height = Home.winfo_screenheight()
...
def ViewForm():
    ...
    btn_reset = Button(LeftViewForm, text="View All Items", command=Reset)
...
def Delete():
    if not tree.selection():
       print("ERROR")
        ...
        result = tkMessageBox.askquestion('Billing Inventory System Disign By CSDT', 'Are you sure you want to delete this record?', icon="warning")
        if result == 'yes':
            curItem = tree.focus()
...
def ShowView():
    global viewform
    viewform = Toplevel()
    viewform.title("Billing Inventory System Disign By CSDT/View Product")
    width = 600
    height = 400
    screen_width = Home.winfo_screenwidth()
    screen_height = Home.winfo_screenheight()
...
def Logout():
    result = tkMessageBox.askquestion('Billing Inventory System Disign By CSDT', 'Are you sure you want to logout?', icon="warning")
    if result == 'yes': 
        admin_id = ""
        root.deiconify()
        Home.destroy()
